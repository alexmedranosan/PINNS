...
class PINN(tf.keras.Model):
    ...
    def train_epochs(self, X_r, X_0, U_0, optim_w, epochs, losses_list, weights_list, predictions_by_epoch=None):
        loss_hist, loss_r_hist, loss_ic_hist = losses_list
        alpha_list, beta_list = weights_list
        ...
        for epoch in  pbar:
            ...
            self.alpha = self.alpha * math.exp(self.lambda_exp * loss_r / (loss_r + loss_ic))
            self.beta = self.beta * math.exp(-self.lambda_exp * loss_ic / (loss_r + loss_ic))
